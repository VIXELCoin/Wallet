...
Below are some notes on how to build Vixel Core for Windows.

The options known to work for building Vixel Core on Windows are:

* On Linux, using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Focal 20.04 is required
and is the platform used to build the Vixel Core Windows release binaries.
...
way. This will install to `c:\workspace\vixel`, for example:

    make install DESTDIR=/mnt/c/workspace/vixel
