...
Currently, 10% of block rewards are set aside for the Vixel DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Vixel in a
credit pool. This credit pool mirrors the available credit supply in Vixel
Platform. When a certain amount of Vixel is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Vixel
Platform. Masternode identities receive the fees and can convert them back to
Vixel by withdrawing them from Vixel Platform.
...
- `bestCLHeightDiff` (`uint32`) is equal to the diff in heights from the mined
  block height
- `creditPoolBalance` (`int64`) is equal to all the VIXELCOIN locked in all previous
  blocks minus all the VIXELCOIN unlocked in all previous blocks plus all the block
...
### Removal of reject network messages from Vixel Core (BIP61)

This feature has been disabled by default since Vixel Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Vixel Core version 20.0.0 there are extra changes:
...
- It is possible to run Vixel Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `VIX/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Vixel Core (BIP61)_](#removal-of-reject-network-messages-from-vixel-core-bip61) above for
...
### Vixel-specific changes in existing RPCs:
...
- The `vixel-cli -getinfo` command now displays the wallet name and balance for
...
Please check `Help -> Command-line options` in Qt wallet or `vixeld --help` for
more information.
...
python test framework and `vixel-cli` tool both take advantage of this, so
for example:

```sh
vixel-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
vixel-cli -named createwallet mywallet load_on_startup=1
```
...
with Vixel’s product needs, such as SegWit and RBF, are excluded from our
...
[DIP-0023]: https://github.com/vixelpay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/vixelpay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/VIXELCoin/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/VIXELCoin/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/VIXELCoin/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/VIXELCoin/compare/v19.3.0...vixelpay:v20.0.0
