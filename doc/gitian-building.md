...
*Setup instructions for a Gitian build of Vixel Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Vixel
...
- [Building Vixel Core](#building-vixel-core)
...
Clone the git repositories for Vixel Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/VIXELCoin
git clone https://github.com/vixelpay/gitian.sigs.git
```
...
in the Vixel Core repository under 'Fetch and create inputs' to install sources which require
...
To build Vixel Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Vixel Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vixel/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/VIXELCoin
...
For example:
```bash
URL=https://github.com/crowning-/vixel.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit vixel=${COMMIT} --url vixel=${URL} ../vixel/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vixel=${COMMIT} --url vixel=${URL} ../vixel/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vixel=${COMMIT} --url vixel=${URL} ../vixel/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the vixel git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=focal on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../vixel/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/vixel
SIGPATH=/some/root/path/vixel-detached-sigs

./bin/gbuild --url vixel=${BTCPATH},signature=${SIGPATH} ../vixel/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vixel-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vixel-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/vixel-osx-build.assert
```
...
[vixel/gitian.sigs](https://github.com/vixelpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
