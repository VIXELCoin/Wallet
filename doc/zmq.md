...
The Vixel Core daemon can be configured to act as a trusted "border
router", implementing the vixel wire protocol and relay, making
...
The ZeroMQ feature in Vixel Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building vixeld:
...
For instance:
    ...
    $ vixeld -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/vixeld.tx.raw \
...
These options can also be provided in vixel.conf.
...
From the perspective of vixeld, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into vixeld directly. Furthermore, no information is
...
using. Vixeld appends an up-counting sequence number to each
