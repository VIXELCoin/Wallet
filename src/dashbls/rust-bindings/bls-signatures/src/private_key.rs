...
impl PrivateKey {
    ...
    #[cfg(feature = "vixel_helpers")]
    pub fn generate_vixel(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "vixel_helpers")]
    pub fn generate_vixel_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_vixel(rng))
            .collect()
    }
