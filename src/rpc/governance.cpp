...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 VIX irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on vixelcentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Vixel features */
    { "vixel",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "vixel",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "vixel",               "gobject",                &gobject,                {} },
    { "vixel",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
