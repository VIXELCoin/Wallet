 ...
 * database.
 *
 * CBlock(hash=00000ffd590b14, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=e0028e, nTime=1710965904, nBits=1e0ffff0, nNonce=676710, vtx=1)
...
{
    const char* pszTimestamp = "VIXELCOIN NET";
    const CScript genesisOutputScript = CScript() << ParseHex("040187dc992e428b4c421306477d679a300a1feea5ee374874722404ca39b67c10186cf340b582bf966f46d4acd94814002cfc11d3bb3c0873e73308c41a147fb4") << OP_CHECKSIG;
...
public:
    CMainParams() {
        ...
        consensus.defaultAssumeValid = uint256S("0x0000060ecbaefc1135b9f2d6c9bd2e8fdecf2a36a3bbe43f315a1b4aa5bb72db"); 
        ...
        genesis = CreateGenesisBlock(1710965904, 676710, 0x1e0ffff0, 1, 50 * COIN);
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x0000060ecbaefc1135b9f2d6c9bd2e8fdecf2a36a3bbe43f315a1b4aa5bb72db"));
        assert(genesis.hashMerkleRoot == uint256S("0x8a599caa8c825f3f33a12636f23a77d16314ba27c75b8cb963bbafd47153aa97"));
        ...
        // Vixel BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x88, 0xB2, 0x1E};
        // Vixel BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x88, 0xAD, 0xE4};

        // Vixel BIP44 coin type is '5'
        nExtCoinType = 5;
        ...
        vSporkAddresses = {"VNcUAcjgnbwDGUecsR6AqsrzZqSVkUg8EY"};
        nMinSporkKeys = 1;

        checkpointData = {
            {
                {0, uint256S("0x0000060ecbaefc1135b9f2d6c9bd2e8fdecf2a36a3bbe43f315a1b4aa5bb72db")},
            }
        };
        ...
        // getchaintxstats 17280 0000060ecbaefc1135b9f2d6c9bd2e8fdecf2a36a3bbe43f315a1b4aa5bb72db
        chainTxData = ChainTxData{
                1710965904, // * UNIX timestamp of last known number of transactions
...
public:
    CTestNetParams() {
        ...
        // By default assume that the signatures in ancestors of this block are valid.
        consensus.defaultAssumeValid = uint256S("0x00000a7639df4064cedee52960d299fdc0609a6c4d8124c2070b3e3b99846083"); 
        ...
        genesis = CreateGenesisBlock(1710965909UL, 137896UL, 0x1e0ffff0, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x00000a7639df4064cedee52960d299fdc0609a6c4d8124c2070b3e3b99846083"));
        assert(genesis.hashMerkleRoot == uint256S("0x8a599caa8c825f3f33a12636f23a77d16314ba27c75b8cb963bbafd47153aa97"));
        ...
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,198);
        // Testnet Vixel BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Testnet Vixel BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Testnet Vixel BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
        ...
        vSporkAddresses = {"VQixcrQPKMYcb3uKeo9BmbeNP8222ewBNK"};
        nMinSporkKeys = 1;

        checkpointData = {
            {
                {0, uint256S("0x00000a7639df4064cedee52960d299fdc0609a6c4d8124c2070b3e3b99846083")},
            }
        };
        ...
        // getchaintxstats 17280 00000a7639df4064cedee52960d299fdc0609a6c4d8124c2070b3e3b99846083
        chainTxData = ChainTxData{
                1710965909, // * UNIX timestamp of last known number of transactions
                0,    // * total number of transactions between genesis and that timestamp
                            //   (the tx=... number in the ChainStateFlushed debug.log lines)
                0,       // * estimated number of transactions per second after that timestamp
        };
...
public:
    explicit CDevNetParams(const ArgsManager& args) {
        ...
        genesis = CreateGenesisBlock(1710965914, 2, 0x207fffff, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x59548a651c362fb2978c921adc65c96cf47b2158ce89662b127e37cacf2d6f13"));
        assert(genesis.hashMerkleRoot == uint256S("0x8a599caa8c825f3f33a12636f23a77d16314ba27c75b8cb963bbafd47153aa97"));
        ...
        checkpointData = (CCheckpointData) {
            {
                { 0, uint256S("0x59548a651c362fb2978c921adc65c96cf47b2158ce89662b127e37cacf2d6f13")},
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        // privKey: 7e5z6yZiQiaBXE7GP6Ugkqi7r1fKDUWevUURXoRrk6uN5Q6vrtK
        vSporkAddresses = {"VPhZdhxZPGExB7Dwms4ajKjHg8WD18PjAM"};
        nMinSporkKeys = 1;
        ...
        // Regtest Vixel addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,70);
        // Regtest Vixel script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,71);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,198);
        // Regtest Vixel BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest Vixel BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest Vixel BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
