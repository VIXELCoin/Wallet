...
const char * const BITCOIN_CONF_FILENAME = "vixel.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //vixel-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Vixel
    // macOS: ~/Library/Application Support/Vixel
    // Unix-like: ~/.vixel
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Vixel";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Vixel";
#else
    // Unix-like
    return pathRet / ".vixel";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty vixel.conf if it does not exist
